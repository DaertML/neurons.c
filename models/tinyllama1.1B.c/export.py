...
def hf_export(llama_model, filepath, group_size=64, dtype=torch.float32):
    ...
    try:
      ...
      hf_state_dict['model.embed_tokens.weight'] = llama_model.embed_tokens.weight.clone().to(dtype)
...
def load_meta_model(model_path):
    ...
    for layer in model.layers:
        ...
        try:
          ...
          layer.attention_norm.weight = nn.Parameter(state_dict[f'model.layers.{i}.input_layernorm.weight'])

        try:
          layer.attention.wq.weight = nn.Parameter(state_dict[f'layers.{i}.attention.wq.weight'])
        except:
          layer.attention.wq.weight = nn.Parameter(state_dict[f'model.layers.{i}.self_attn.q_proj.weight'])

        try:
          layer.attention.wk.weight = nn.Parameter(state_dict[f'layers.{i}.attention.wk.weight'])
        except:
          layer.attention.wk.weight = nn.Parameter(state_dict[f'model.layers.{i}.self_attn.k_proj.weight'])

        try:
          layer.attention.wv.weight = nn.Parameter(state_dict[f'layers.{i}.attention.wv.weight'])
        except:
          layer.attention.wv.weight = nn.Parameter(state_dict[f'model.layers.{i}.self_attn.v_proj.weight'])

        try:
          layer.attention.wo.weight = nn.Parameter(state_dict[f'layers.{i}.attention.wo.weight'])
        except:
          layer.attention.wo.weight = nn.Parameter(state_dict[f'model.layers.{i}.self_attn.o_proj.weight'])

        try:      
          layer.ffn_norm.weight = nn.Parameter(state_dict[f'layers.{i}.ffn_norm.weight'])
        except:
          layer.ffn_norm.weight = nn.Parameter(state_dict[f'model.layers.{i}.post_attention_layernorm.weight'])

        try:
          layer.feed_forward.w1.weight = nn.Parameter(state_dict[f'layers.{i}.feed_forward.w1.weight'])
        except:
          layer.feed_forward.w1.weight = nn.Parameter(state_dict[f'model.layers.{i}.mlp.gate_proj.weight'])

        try:
          layer.feed_forward.w2.weight = nn.Parameter(state_dict[f'layers.{i}.feed_forward.w2.weight'])
        except:
          layer.feed_forward.w2.weight = nn.Parameter(state_dict[f'model.layers.{i}.mlp.up_proj.weight'])

        try:
          layer.feed_forward.w3.weight = nn.Parameter(state_dict[f'layers.{i}.feed_forward.w3.weight'])
        except:
          layer.feed_forward.w2.weight = nn.Parameter(state_dict[f'model.layers.{i}.mlp.down_proj.weight'])
